...
import { feedingsAnalyticsKey } from "@/components/day-view/feedingsKey";
...
async function fetchAnalytics(tz: string): Promise<AnalyticsResponse> {
  const r = await fetch("/api/feedings/analytics", {
    cache: "no-store",
    headers: tzHeaders(tz),
  });
  if (!r.ok) throw new Error("feedings analytics fetch failed");
  return r.json();
}
